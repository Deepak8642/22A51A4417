"""
PDF → Excel  ·  FINAL ACCURATE CONVERTER
==========================================
Converts any PDF to Excel preserving structure, alignment, and formatting.
...
# ─── core: chars → visual lines → segments ────────────────────────────────────
...
def chars_to_segs(chs, gap=8):
    """Split chars into x-gap segments. Merge dot-leader chars into parent segment."""
    if not chs: return []
    ...
    for s in segs:
        ...
        if not txt: continue
        # Skip pure dot-leaders (". . . . . " style) — merge into previous
        is_dots = all(c["text"] in (". ", ".", " ") for c in s)
        if is_dots and result:
            # append dots to previous segment's text
            result[-1]["text"] += " " + txt
            result[-1]["x1"] = float(s[-1]["x1"])
            continue
        result.append(dict(
            x0=float(s[0]["x0"]), x1=float(s[-1]["x1"]), text=txt,
...
def get_fills(page):
    ...
    for r in page.rects:
        ...
        if not isinstance(c, (list, tuple)) or len(c) < 3: continue
        ...
        out.append((float(r["top"]), float(r["bottom"]),
                    float(r["x0"]),  float(r["x1"]), kind))
    return out

def fill_at(y, fills, tol=3):
    # Only return kind if the rect spans most of the page width (true header)
    for top, bot, rx0, rx1, kind in fills:
        if top - tol <= y <= bot + tol and (rx1 - rx0) > 100:
            return kind
    return None

# ─── column slot map ──────────────────────────────────────────────────────────
def build_slots(all_segs, page_width, snap=5):
...
def write_page(ws, page, page_images):
    ...
    for xl_row, (y, chs) in enumerate(vis_lines, start=1):
        ...
        ws.row_dimensions[xl_row].height = max(10, round(sz * 1.7))

        if not segs: continue

        # ── BLACK PART HEADER ────────────────────────────────────────────────
        if kind == "black":
            # Only make it a header if the line contains "Part" keyword
            has_part = any("Part" in s["text"] or "Supplemental" in s["text"] for s in segs)
            if has_part:
                ...
                if n_slots > 1:
                    try: ws.merge_cells(start_row=xl_row, start_column=1,
...
def convert(pdf_path, out_path):
    ...
    if not images: print("  [INFO] pip install pymupdf  to keep embedded images")
    wb = Workbook()
    with pdfplumber.open(pdf_path) as pdf:
        for pn, page in enumerate(pdf.pages):
            ...
            write_page(ws, page, [im for im in images if im["page"] == pn])
    wb.save(out_path)
    print(f"\n  ✓  Saved → {out_path}\n")
