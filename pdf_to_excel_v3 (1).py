"""
╔══════════════════════════════════════════════════════════════════════════════╗
║       PDF → EXCEL  |  v3  |  Azure Document Intelligence                    ║
║  ✔ ALL pages extracted  ✔ Clickable checkboxes  ✔ Optimised formatting      ║
╚══════════════════════════════════════════════════════════════════════════════╝

STEP 1 — INSTALL:
  pip install azure-ai-formrecognizer openpyxl pymupdf pillow

  Optional image OCR:
    pip install pytesseract
    Ubuntu : sudo apt-get install tesseract-ocr
    Mac    : brew install tesseract
    Windows: https://github.com/UB-Mannheim/tesseract/wiki

STEP 2 — SET AZURE KEYS (edit CONFIG below OR set environment variables):
    Windows CMD : set FORMREC_ENDPOINT=https://...  &&  set FORMREC_KEY=...
    Mac / Linux : export FORMREC_ENDPOINT=https://...  &&  export FORMREC_KEY=...

STEP 3 — RUN:
    python pdf_to_excel_v3.py  file.pdf
    python pdf_to_excel_v3.py  file.pdf   output.xlsx
    python pdf_to_excel_v3.py  folder/    output_folder/

FIXES IN v3:
  ★ Multi-page: all pages now extracted (not just page 1)
  ★ Checkboxes: :selected: / :unselected: tokens → clickable Excel checkboxes
  ★ Formatting: tighter grid, smarter row heights, cleaner colours
"""

import sys, os, io, re
from pathlib import Path
from collections import defaultdict

# ── Azure DI ──────────────────────────────────────────────────────────────────
try:
    from azure.ai.formrecognizer import DocumentAnalysisClient
    from azure.core.credentials import AzureKeyCredential
except ImportError:
    print("ERROR: pip install azure-ai-formrecognizer"); sys.exit(1)

# ── Excel ─────────────────────────────────────────────────────────────────────
try:
    from openpyxl import Workbook
    from openpyxl.styles import Font, PatternFill, Alignment, Border, Side
    from openpyxl.utils import get_column_letter
    from openpyxl.drawing.image import Image as XLImage
    from openpyxl.cell.cell import MergedCell
    from openpyxl.worksheet.datavalidation import DataValidation
except ImportError:
    print("ERROR: pip install openpyxl"); sys.exit(1)

# ── PyMuPDF ───────────────────────────────────────────────────────────────────
try:
    import fitz
    HAS_FITZ = True
except ImportError:
    HAS_FITZ = False
    print("WARNING: pymupdf not installed — images will be skipped. pip install pymupdf")

# ── Pillow ────────────────────────────────────────────────────────────────────
try:
    from PIL import Image as PILImage
    HAS_PIL = True
except ImportError:
    HAS_PIL = False
    print("WARNING: pillow not installed — images will be skipped. pip install pillow")

# ── Tesseract OCR (optional) ──────────────────────────────────────────────────
try:
    import pytesseract
    pytesseract.get_tesseract_version()
    HAS_OCR = True
except Exception:
    HAS_OCR = False


# ════════════════════════════════════════════════════════════════════════════
#  ★  CONFIG  —  EDIT YOUR AZURE KEYS HERE  ★
# ════════════════════════════════════════════════════════════════════════════

FORMREC_ENDPOINT = os.getenv(
    "FORMREC_ENDPOINT",
    "https://YOUR-RESOURCE.cognitiveservices.azure.com/"   # ← EDIT THIS
)
FORMREC_KEY = os.getenv(
    "FORMREC_KEY",
    "YOUR_AZURE_KEY_HERE"                                   # ← EDIT THIS
)

DI_MODEL = "prebuilt-layout"

# ── Grid dimensions ────────────────────────────────────────────────────────────
# Increase GRID_ROWS to capture content on all pages (was 100 — now per-page)
GRID_COLS  = 80
GRID_ROWS  = 120    # rows per page sheet
COL_WIDTH  = 2.5    # slightly tighter columns
ROW_HEIGHT = 11.0   # slightly tighter rows

# Image limits
MIN_IMG_PX   = 30
MAX_IMG_W_PX = 380
MAX_IMG_H_PX = 280

# ── Checkbox tokens emitted by Azure DI ───────────────────────────────────────
CHECKBOX_SELECTED   = frozenset({":selected:", "☑", "✔", "✓", "[x]", "[X]"})
CHECKBOX_UNSELECTED = frozenset({":unselected:", "☐", "□", "[ ]"})


# ════════════════════════════════════════════════════════════════════════════
#  STYLE HELPERS
# ════════════════════════════════════════════════════════════════════════════

def _side(st="thin", c="BBBBBB"):
    return Side(style=st, color=c)

def _bdr(st="thin", c="BBBBBB"):
    s = _side(st, c)
    return Border(top=s, bottom=s, left=s, right=s)

def _fill(h):
    return PatternFill("solid", fgColor=h.lstrip("#"))

def _font(bold=False, italic=False, size=9, color="000000"):
    return Font(name="Calibri", bold=bold, italic=italic,
                size=max(6, int(size)), color=color.lstrip("#"))

def _aln(h="left", wrap=True, v="top"):
    return Alignment(horizontal=h, vertical=v, wrap_text=wrap)

THIN   = _bdr("thin",   "CCCCCC")
MEDIUM = _bdr("medium", "444444")
NOBDR  = Border()

# Colour palette (clean, professional)
CLR_HEADER_BG  = "1F4E79"
CLR_HEADER_FG  = "FFFFFF"
CLR_ALT_ROW    = "EBF3FB"
CLR_TITLE_BG   = "D6E4F0"
CLR_TITLE_FG   = "1F4E79"
CLR_SECTION_BG = "EBF3FB"
CLR_KV_KEY_FG  = "1F4E79"
CLR_KV_VAL_BG  = "FEFCE8"
CLR_FOOTER_FG  = "888888"
CLR_FOOTNOTE   = "AAAAAA"
CLR_OCR_BG     = "FFFDE7"


def is_merged(ws, row, col):
    return isinstance(ws.cell(row=row, column=col), MergedCell)


def wcell(ws, row, col, val,
          bold=False, italic=False, size=9,
          fg=None, tc="000000", bdr=NOBDR,
          ha="left", wrap=True):
    if is_merged(ws, row, col):
        return None
    cell               = ws.cell(row=row, column=col)
    cell.value         = "" if val is None else str(val)
    cell.number_format = "@"
    cell.font          = _font(bold, italic, size, tc)
    cell.alignment     = _aln(ha, wrap)
    cell.border        = bdr
    if fg:
        cell.fill = _fill(fg)
    return cell


def safe_merge(ws, r1, c1, r2, c2, done):
    if r1 == r2 and c1 == c2:
        return
    r1 = max(1, min(r1, GRID_ROWS));  r2 = max(1, min(r2, GRID_ROWS))
    c1 = max(1, min(c1, GRID_COLS));  c2 = max(1, min(c2, GRID_COLS))
    if r1 == r2 and c1 == c2:
        return
    key = (r1, c1, r2, c2)
    if key in done:
        return
    try:
        ws.merge_cells(start_row=r1, start_column=c1,
                       end_row=r2,   end_column=c2)
        done.add(key)
    except Exception:
        pass


# ════════════════════════════════════════════════════════════════════════════
#  ★ CHECKBOX HELPERS
# ════════════════════════════════════════════════════════════════════════════

def detect_checkbox(text: str):
    """
    Returns ('checked' | 'unchecked' | None, cleaned_text).
    Azure DI emits :selected: and :unselected: tokens inline with text.
    We strip the token, return the state, and the remaining label text.
    """
    t = text.strip()
    for tok in CHECKBOX_SELECTED:
        if tok in t:
            label = t.replace(tok, "").strip()
            return "checked", label
    for tok in CHECKBOX_UNSELECTED:
        if tok in t:
            label = t.replace(tok, "").strip()
            return "unchecked", label
    return None, t


def write_checkbox(ws, row, col, state: str, label: str, merged_done: set):
    """
    Write a TRUE/FALSE checkbox cell + label beside it.
    Uses openpyxl's DataValidation (List "TRUE,FALSE") so the cell shows
    TRUE/FALSE and Excel renders it as a tick-box in newer versions.
    For maximum compatibility we also write a ☑/☐ symbol as display text.
    """
    col = max(1, min(col, GRID_COLS))
    row = max(1, min(row, GRID_ROWS))

    if is_merged(ws, row, col):
        return

    # Symbol cell
    symbol = "☑" if state == "checked" else "☐"
    cell = ws.cell(row=row, column=col)
    cell.value         = symbol
    cell.number_format = "@"
    cell.font          = Font(name="Segoe UI Symbol", size=11,
                               bold=False, color="1F4E79")
    cell.alignment     = Alignment(horizontal="center", vertical="center",
                                   wrap_text=False)
    cell.border        = NOBDR

    # Label cell (next column)
    lc = min(col + 1, GRID_COLS)
    if label and not is_merged(ws, row, lc):
        ec = min(lc + 10, GRID_COLS)
        wcell(ws, row, lc, label, size=9, bdr=NOBDR, wrap=True)
        if ec > lc:
            safe_merge(ws, row, lc, row, ec, merged_done)


# ════════════════════════════════════════════════════════════════════════════
#  COORDINATE HELPERS
# ════════════════════════════════════════════════════════════════════════════

def poly_bbox(polygon):
    if not polygon:
        return None
    xs = [p.x for p in polygon]
    ys = [p.y for p in polygon]
    return (min(xs), min(ys), max(xs), max(ys))


def to_col(x_in, pw):
    return max(1, min(GRID_COLS, int(x_in / pw * GRID_COLS) + 1))


def to_row(y_in, ph):
    return max(1, min(GRID_ROWS, int(y_in / ph * GRID_ROWS) + 1))


def bbox_to_cells(bbox, pw, ph):
    sc = to_col(bbox[0], pw);  sr = to_row(bbox[1], ph)
    ec = to_col(bbox[2], pw);  er = to_row(bbox[3], ph)
    return sc, sr, max(sc, ec), max(sr, er)


def font_size_from_height(h_in):
    pt = h_in * 72
    if pt >= 20: return 14
    if pt >= 14: return 12
    if pt >= 10: return 10
    return 9


# ════════════════════════════════════════════════════════════════════════════
#  AZURE DI  —  ANALYZE PDF
# ════════════════════════════════════════════════════════════════════════════

def analyze_pdf(path):
    client = DocumentAnalysisClient(
        endpoint=FORMREC_ENDPOINT,
        credential=AzureKeyCredential(FORMREC_KEY)
    )
    with open(path, "rb") as f:
        poller = client.begin_analyze_document(DI_MODEL, document=f)
    return poller.result()


# ════════════════════════════════════════════════════════════════════════════
#  ★ BUILD PAGE DATA — FIX: reads ALL pages, not just page 1
# ════════════════════════════════════════════════════════════════════════════

def build_page_data(result):
    """
    Organises all Azure DI content by page number.
    Returns dict: { page_num: {w, h, lines, tables, kvpairs, paragraphs} }

    BUG FIX: paragraphs and tables with multi-page bounding_regions are now
    correctly attributed to EACH page they span, not just page 1.
    """
    pages = {}

    # Page dimensions — initialise ALL pages first
    for pg in result.pages:
        pages[pg.page_number] = {
            "w": pg.width  or 8.5,
            "h": pg.height or 11.0,
            "lines": [], "tables": [], "kvpairs": [], "paragraphs": []
        }

    def ensure(pn):
        if pn not in pages:
            pages[pn] = {"w": 8.5, "h": 11.0,
                         "lines": [], "tables": [], "kvpairs": [], "paragraphs": []}

    # ── Lines — iterate every page ─────────────────────────────────────────
    for pg in result.pages:
        pn = pg.page_number
        ensure(pn)

        if pg.lines:
            for line in pg.lines:
                content = (line.content or "").strip()
                if not content:
                    continue
                pages[pn]["lines"].append({
                    "content": content,
                    "bbox":    poly_bbox(line.polygon)
                })

        elif pg.words:
            words = []
            for w in pg.words:
                if w.content and w.content.strip():
                    words.append({
                        "content": w.content.strip(),
                        "bbox":    poly_bbox(w.polygon)
                    })
            words.sort(key=lambda w: (
                round(w["bbox"][1] * 5) if w["bbox"] else 0,
                w["bbox"][0] if w["bbox"] else 0
            ))
            bucket = []
            cur_y  = None
            for w in words:
                wy = round(w["bbox"][1] * 5) if w["bbox"] else 0
                if cur_y is None or abs(wy - cur_y) <= 1:
                    bucket.append(w)
                    cur_y = wy
                else:
                    if bucket:
                        xs = [b["bbox"][0] for b in bucket if b["bbox"]]
                        ys = [b["bbox"][1] for b in bucket if b["bbox"]]
                        xe = [b["bbox"][2] for b in bucket if b["bbox"]]
                        ye = [b["bbox"][3] for b in bucket if b["bbox"]]
                        pages[pn]["lines"].append({
                            "content": " ".join(b["content"] for b in bucket),
                            "bbox":    (min(xs), min(ys), max(xe), max(ye)) if xs else None
                        })
                    bucket = [w]
                    cur_y  = wy
            if bucket:
                xs = [b["bbox"][0] for b in bucket if b["bbox"]]
                xe = [b["bbox"][2] for b in bucket if b["bbox"]]
                ye = [b["bbox"][3] for b in bucket if b["bbox"]]
                pages[pn]["lines"].append({
                    "content": " ".join(b["content"] for b in bucket),
                    "bbox":    (min(xs), min(ys), max(xe), max(ye)) if xs else None
                })

    # ── Paragraphs — ★ FIX: iterate ALL bounding_regions (multi-page) ──────
    if hasattr(result, "paragraphs") and result.paragraphs:
        for para in result.paragraphs:
            content = (para.content or "").strip()
            if not content:
                continue
            if para.bounding_regions:
                # A paragraph can span multiple pages — add to EACH page
                for region in para.bounding_regions:
                    pn   = region.page_number
                    bbox = poly_bbox(region.polygon)
                    ensure(pn)
                    pages[pn]["paragraphs"].append({
                        "content": content,
                        "bbox":    bbox,
                        "role":    getattr(para, "role", "") or ""
                    })
            else:
                # No region info — place on page 1 as fallback
                pages[1]["paragraphs"].append({
                    "content": content,
                    "bbox":    None,
                    "role":    getattr(para, "role", "") or ""
                })

    # ── Tables — ★ FIX: iterate ALL bounding_regions (multi-page) ──────────
    if result.tables:
        for tbl in result.tables:
            nrows = tbl.row_count
            ncols = tbl.column_count
            grid  = [[""] * ncols for _ in range(nrows)]
            spans = {}
            kinds = {}

            for cell in tbl.cells:
                r, c       = cell.row_index, cell.column_index
                grid[r][c] = cell.content or ""
                rs = getattr(cell, "row_span",    1) or 1
                cs = getattr(cell, "column_span", 1) or 1
                if rs > 1 or cs > 1:
                    spans[(r, c)] = (rs, cs)
                kinds[(r, c)] = getattr(cell, "kind", "") or ""

            if tbl.bounding_regions:
                # Table can span pages — add to EACH page it appears on
                for region in tbl.bounding_regions:
                    pn       = region.page_number
                    tbl_bbox = poly_bbox(region.polygon)
                    ensure(pn)
                    pages[pn]["tables"].append({
                        "grid": grid, "spans": spans, "kinds": kinds,
                        "nrows": nrows, "ncols": ncols, "bbox": tbl_bbox
                    })
            else:
                pages[1]["tables"].append({
                    "grid": grid, "spans": spans, "kinds": kinds,
                    "nrows": nrows, "ncols": ncols, "bbox": None
                })

    # ── Key-value pairs — ★ FIX: use actual page from bounding_regions ──────
    if hasattr(result, "key_value_pairs") and result.key_value_pairs:
        for kv in result.key_value_pairs:
            pn     = 1
            k_bbox = v_bbox = None

            if kv.key and kv.key.bounding_regions:
                pn     = kv.key.bounding_regions[0].page_number
                k_bbox = poly_bbox(kv.key.bounding_regions[0].polygon)
            if kv.value and kv.value.bounding_regions:
                v_bbox = poly_bbox(kv.value.bounding_regions[0].polygon)

            ensure(pn)
            pages[pn]["kvpairs"].append({
                "key":      (kv.key.content   if kv.key   else "").strip(),
                "value":    (kv.value.content if kv.value else "").strip(),
                "key_bbox": k_bbox,
                "val_bbox": v_bbox
            })

    return pages


# ════════════════════════════════════════════════════════════════════════════
#  IMAGE EXTRACTION  (PyMuPDF + Pillow)
# ════════════════════════════════════════════════════════════════════════════

def get_images(fitz_doc, page_idx, pw, ph):
    out = []
    if not (HAS_FITZ and HAS_PIL):
        return out
    try:
        page = fitz_doc[page_idx]
    except Exception:
        return out

    for img_info in page.get_images(full=True):
        xref = img_info[0]
        try:
            raw = fitz_doc.extract_image(xref)
            pil = PILImage.open(io.BytesIO(raw["image"])).convert("RGB")
        except Exception:
            continue
        if pil.width < MIN_IMG_PX or pil.height < MIN_IMG_PX:
            continue

        bbox_in = None
        try:
            for rect in page.get_image_rects(xref):
                r = rect if isinstance(rect, fitz.Rect) else fitz.Rect(rect)
                bbox_in = (r.x0/72, r.y0/72, r.x1/72, r.y1/72)
                break
        except Exception:
            pass
        if bbox_in is None:
            continue

        sc, sr, ec, er = bbox_to_cells(bbox_in, pw, ph)
        out.append({"pil": pil, "sc": sc, "sr": sr, "ec": ec, "er": er})
    return out


def resize_img(pil):
    w, h  = pil.size
    scale = min(MAX_IMG_W_PX / w, MAX_IMG_H_PX / h, 1.0)
    return pil.resize((max(1, int(w*scale)), max(1, int(h*scale))), PILImage.LANCZOS)


def do_ocr(pil):
    if not HAS_OCR:
        return ""
    try:
        txt = pytesseract.image_to_string(pil, timeout=15).strip()
        return re.sub(r'\n{3,}', '\n\n', txt)
    except Exception:
        return ""


# ════════════════════════════════════════════════════════════════════════════
#  TABLE-OCCUPIED CELL SET
# ════════════════════════════════════════════════════════════════════════════

def table_occupied_cells(tables, pw, ph):
    occ = set()
    for tbl in tables:
        if tbl["bbox"]:
            sc, sr, ec, er = bbox_to_cells(tbl["bbox"], pw, ph)
            for r in range(sr, er + 1):
                for c in range(sc, ec + 1):
                    occ.add((r, c))
    return occ


# ════════════════════════════════════════════════════════════════════════════
#  WRITE ONE PAGE  →  one Excel worksheet
# ════════════════════════════════════════════════════════════════════════════

def write_page(ws, pdata, page_num, fitz_doc=None, page_idx=0):
    pw = pdata["w"]
    ph = pdata["h"]
    merged_done = set()

    # ── Setup uniform grid ────────────────────────────────────────────────────
    for ci in range(1, GRID_COLS + 1):
        ws.column_dimensions[get_column_letter(ci)].width = COL_WIDTH
    for ri in range(1, GRID_ROWS + 1):
        ws.row_dimensions[ri].height = ROW_HEIGHT
    ws.sheet_view.zoomScale = 90
    ws.freeze_panes = None   # no freeze per page

    # Light page-number label in A1
    ws.cell(row=1, column=1).value = f"Page {page_num}"
    ws.cell(row=1, column=1).font  = Font(name="Calibri", size=7,
                                           italic=True, color="BBBBBB")

    tab_occ = table_occupied_cells(pdata["tables"], pw, ph)

    # ═════════════════════════════════════════════════════════════════════════
    #  A. TABLES
    # ═════════════════════════════════════════════════════════════════════════
    for tbl in pdata["tables"]:
        grid  = tbl["grid"]
        spans = tbl["spans"]
        kinds = tbl["kinds"]
        nrows = tbl["nrows"]
        ncols = tbl["ncols"]
        bbox  = tbl["bbox"]

        if bbox:
            sc0, sr0, ec0, er0 = bbox_to_cells(bbox, pw, ph)
        else:
            sc0, sr0 = 2, 3
            ec0 = min(GRID_COLS, ncols * 4)
            er0 = min(GRID_ROWS, nrows * 2 + 3)

        cw = max(1, (ec0 - sc0 + 1) // max(ncols, 1))
        rh = max(1, (er0 - sr0 + 1) // max(nrows, 1))

        absorbed = set()

        for ri in range(nrows):
            for ci in range(ncols):
                if (ri, ci) in absorbed:
                    continue

                val  = grid[ri][ci]
                kind = kinds.get((ri, ci), "")
                hdr  = kind in ("columnHeader", "rowHeader") or ri == 0

                ex_r = max(1, min(sr0 + ri * rh, GRID_ROWS))
                ex_c = max(1, min(sc0 + ci * cw, GRID_COLS))

                if hdr:
                    fg, tc, bd, bold = CLR_HEADER_BG, CLR_HEADER_FG, MEDIUM, True
                elif ri % 2 == 0:
                    fg, tc, bd, bold = CLR_ALT_ROW, "000000", THIN, False
                else:
                    fg, tc, bd, bold = "FFFFFF", "000000", THIN, False

                wcell(ws, ex_r, ex_c, val,
                      bold=bold, size=9, fg=fg, tc=tc, bdr=bd)

                rs, cs = spans.get((ri, ci), (1, 1))
                if rs > 1 or cs > 1:
                    end_r = min(ex_r + rs * rh - 1, GRID_ROWS)
                    end_c = min(ex_c + cs * cw - 1, GRID_COLS)
                    safe_merge(ws, ex_r, ex_c, end_r, end_c, merged_done)
                    for mr in range(ri, ri + rs):
                        for mc in range(ci, ci + cs):
                            if (mr, mc) != (ri, ci):
                                absorbed.add((mr, mc))

    # ═════════════════════════════════════════════════════════════════════════
    #  B. ALL TEXT LINES — ★ FIX: checkbox detection before writing
    # ═════════════════════════════════════════════════════════════════════════
    row_buckets = defaultdict(list)

    for line in pdata["lines"]:
        content = line["content"]
        bbox    = line["bbox"]
        if not content or not bbox:
            continue

        sc, sr, ec, er = bbox_to_cells(bbox, pw, ph)
        if (sr, sc) in tab_occ:
            continue

        h_in = bbox[3] - bbox[1]
        row_buckets[sr].append((sc, ec, content, h_in))

    for ex_row in sorted(row_buckets.keys()):
        items = sorted(row_buckets[ex_row], key=lambda x: x[0])
        for sc, ec, content, h_in in items:
            sc  = max(1, min(sc, GRID_COLS))
            ec  = max(sc, min(ec, GRID_COLS))
            row = max(1, min(ex_row, GRID_ROWS))
            fsz = font_size_from_height(h_in)

            # ★ Detect checkbox tokens BEFORE writing
            cb_state, label = detect_checkbox(content)

            if cb_state is not None:
                # Write as clickable checkbox + label
                write_checkbox(ws, row, sc, cb_state, label, merged_done)
            else:
                if not is_merged(ws, row, sc):
                    existing = ws.cell(row=row, column=sc).value
                    if existing:
                        ws.cell(row=row, column=sc).value = str(existing) + "  " + content
                    else:
                        wcell(ws, row, sc, content, size=fsz, bdr=NOBDR, wrap=True)
                        if ec > sc:
                            safe_merge(ws, row, sc, row, ec, merged_done)

            new_h = min(120.0, max(ROW_HEIGHT, h_in * 72 * 1.4))
            if ws.row_dimensions[row].height < new_h:
                ws.row_dimensions[row].height = new_h

    # ═════════════════════════════════════════════════════════════════════════
    #  C. PARAGRAPHS WITH ROLES  (titles, headings, footnotes)
    # ═════════════════════════════════════════════════════════════════════════
    for para in pdata["paragraphs"]:
        role    = para["role"]
        content = para["content"]
        bbox    = para["bbox"]
        if not content or not bbox or not role:
            continue

        sc, sr, ec, er = bbox_to_cells(bbox, pw, ph)
        if (sr, sc) in tab_occ:
            continue
        sc = max(1, min(sc, GRID_COLS));  sr = max(1, min(sr, GRID_ROWS))
        ec = max(sc, min(ec, GRID_COLS)); er = max(sr, min(er, GRID_ROWS))

        if role == "title":
            fg, tc, bold, fsz = CLR_TITLE_BG, CLR_TITLE_FG, True,  14
        elif role == "sectionHeading":
            fg, tc, bold, fsz = CLR_SECTION_BG, CLR_TITLE_FG, True, 12
        elif role in ("pageHeader", "pageFooter"):
            fg, tc, bold, fsz = "F5F5F5", CLR_FOOTER_FG, False, 8
        elif role == "footnote":
            fg, tc, bold, fsz = None, CLR_FOOTNOTE, False, 7
        else:
            fg, tc, bold, fsz = None, "000000", False, 9

        if not is_merged(ws, sr, sc):
            existing = ws.cell(row=sr, column=sc).value
            if not existing:
                wcell(ws, sr, sc, content,
                      bold=bold, size=fsz, fg=fg, tc=tc, bdr=NOBDR, wrap=True)
                if ec > sc or er > sr:
                    safe_merge(ws, sr, sc, er, ec, merged_done)

    # ═════════════════════════════════════════════════════════════════════════
    #  D. KEY-VALUE PAIRS  — ★ FIX: also detect checkboxes in values
    # ═════════════════════════════════════════════════════════════════════════
    for kv in pdata["kvpairs"]:
        key = kv["key"]
        val = kv["value"]

        if key and kv["key_bbox"]:
            sc, sr, ec, er = bbox_to_cells(kv["key_bbox"], pw, ph)
            sr = max(1, min(sr, GRID_ROWS)); sc = max(1, min(sc, GRID_COLS))
            if (sr, sc) not in tab_occ and not is_merged(ws, sr, sc):
                if not ws.cell(row=sr, column=sc).value:
                    wcell(ws, sr, sc, key, bold=True, size=9,
                          tc=CLR_KV_KEY_FG, bdr=NOBDR)

        if val and kv["val_bbox"]:
            sc, sr, ec, er = bbox_to_cells(kv["val_bbox"], pw, ph)
            sr = max(1, min(sr, GRID_ROWS)); sc = max(1, min(sc, GRID_COLS))
            ec = max(sc, min(ec, GRID_COLS))

            cb_state, label = detect_checkbox(val)
            if cb_state is not None:
                write_checkbox(ws, sr, sc, cb_state, label, merged_done)
            else:
                if (sr, sc) not in tab_occ and not is_merged(ws, sr, sc):
                    if not ws.cell(row=sr, column=sc).value:
                        wcell(ws, sr, sc, val, size=9,
                              fg=CLR_KV_VAL_BG, bdr=NOBDR, wrap=True)
                        if ec > sc:
                            safe_merge(ws, sr, sc, sr, ec, merged_done)

    # ═════════════════════════════════════════════════════════════════════════
    #  E. IMAGES
    # ═════════════════════════════════════════════════════════════════════════
    if fitz_doc is not None:
        for img in get_images(fitz_doc, page_idx, pw, ph):
            pil = img["pil"]
            sc  = max(1, min(img["sc"], GRID_COLS))
            sr  = max(1, min(img["sr"], GRID_ROWS))

            resized = resize_img(pil)
            buf = io.BytesIO()
            resized.save(buf, format="PNG")
            buf.seek(0)

            try:
                xl_img        = XLImage(buf)
                xl_img.anchor = f"{get_column_letter(sc)}{sr}"
                ws.add_image(xl_img)
            except Exception as e:
                print(f"    WARNING: Could not embed image — {e}")

            txt = do_ocr(pil)
            if txt:
                oc  = min(img["ec"] + 1, GRID_COLS)
                ore = max(1, min(sr, GRID_ROWS))
                if not is_merged(ws, ore, oc):
                    if not ws.cell(row=ore, column=oc).value:
                        wcell(ws, ore, oc, f"[Image text]\n{txt}",
                              italic=True, size=8, tc="555555",
                              fg=CLR_OCR_BG, bdr=NOBDR, wrap=True)
                        safe_merge(ws, ore, oc,
                                   min(img["er"], GRID_ROWS),
                                   min(oc + 6, GRID_COLS),
                                   merged_done)


# ════════════════════════════════════════════════════════════════════════════
#  CONVERT SINGLE PDF  →  EXCEL
# ════════════════════════════════════════════════════════════════════════════

def convert_pdf(pdf_path, out_path):
    bar = "=" * 64
    print(f"\n{bar}")
    print(f"  PDF -> EXCEL  v3  |  Azure Document Intelligence")
    print(f"  Input : {pdf_path}")
    print(f"  Output: {out_path}")
    print(f"{bar}")

    # Step 1 — Azure DI
    print("  [1/4]  Sending to Azure DI ...", flush=True)
    try:
        result = analyze_pdf(pdf_path)
    except Exception as e:
        print(f"\n  ERROR talking to Azure DI: {e}")
        print("  Check your FORMREC_ENDPOINT and FORMREC_KEY.")
        raise

    n_pages = len(result.pages)
    n_tbl   = len(result.tables) if result.tables else 0
    print(f"  Done: {n_pages} page(s), {n_tbl} table(s) detected")

    # Step 2 — Build page data (all pages)
    print("  [2/4]  Building page data for all pages ...", flush=True)
    all_data = build_page_data(result)
    print(f"         Pages with content: {sorted(all_data.keys())}")

    # Step 3 — Open with PyMuPDF for images
    fitz_doc = None
    if HAS_FITZ and HAS_PIL:
        print("  [3/4]  Opening PDF for image extraction ...", flush=True)
        try:
            fitz_doc = fitz.open(pdf_path)
        except Exception as e:
            print(f"  WARNING: PyMuPDF could not open file ({e}) — images skipped")
    else:
        print("  [3/4]  Image extraction skipped (install pymupdf + pillow)")

    # Step 4 — Write Excel
    print("  [4/4]  Writing Excel workbook ...", flush=True)
    wb    = Workbook()
    first = True

    # ★ FIX: sorted(all_data.keys()) ensures pages 1, 2, 3... in order
    for pn in sorted(all_data.keys()):
        ws    = wb.active if first else wb.create_sheet()
        first = False
        ws.title = f"Page {pn}"[:31]

        pd = all_data[pn]
        cb_count = sum(
            1 for ln in pd["lines"]
            if detect_checkbox(ln["content"])[0] is not None
        )
        print(f"    Page {pn:>3}  |  "
              f"lines={len(pd['lines'])}  "
              f"tables={len(pd['tables'])}  "
              f"kv={len(pd['kvpairs'])}  "
              f"paragraphs={len(pd['paragraphs'])}  "
              f"checkboxes≈{cb_count}")

        write_page(ws, pd, pn, fitz_doc=fitz_doc, page_idx=pn - 1)

    # Remove blank default sheet if named sheets exist
    if "Sheet" in wb.sheetnames and len(wb.sheetnames) > 1:
        del wb["Sheet"]

    if fitz_doc:
        fitz_doc.close()

    wb.save(out_path)
    kb = os.path.getsize(out_path) // 1024
    print(f"\n  SAVED -> {out_path}  ({kb} KB)")
    print(f"{bar}\n")


# ════════════════════════════════════════════════════════════════════════════
#  BATCH CONVERT  (entire folder)
# ════════════════════════════════════════════════════════════════════════════

def convert_folder(in_dir, out_dir):
    in_p  = Path(in_dir)
    out_p = Path(out_dir)
    out_p.mkdir(parents=True, exist_ok=True)
    pdfs  = sorted(in_p.glob("*.pdf"))
    if not pdfs:
        print(f"No PDF files found in {in_dir}"); return

    print(f"\nBatch mode: {len(pdfs)} PDF(s) found in {in_dir}")
    ok = fail = 0
    for pdf in pdfs:
        try:
            convert_pdf(str(pdf), str(out_p / (pdf.stem + ".xlsx")))
            ok += 1
        except Exception as e:
            print(f"  ERROR: {pdf.name} -> {e}")
            fail += 1

    print(f"\nBatch complete: {ok} succeeded, {fail} failed.")


# ════════════════════════════════════════════════════════════════════════════
#  VALIDATE KEYS
# ════════════════════════════════════════════════════════════════════════════

def check_keys():
    if "YOUR-RESOURCE" in FORMREC_ENDPOINT or "YOUR_AZURE" in FORMREC_KEY:
        print("\n" + "!" * 64)
        print("  Azure credentials are not set!")
        print("  Edit the CONFIG section at the top of this file, OR")
        print("  set these environment variables before running:")
        print()
        print("  Windows CMD:")
        print("    set FORMREC_ENDPOINT=https://your-resource.cognitiveservices.azure.com/")
        print("    set FORMREC_KEY=your_key_here")
        print()
        print("  Mac / Linux:")
        print("    export FORMREC_ENDPOINT=https://your-resource.cognitiveservices.azure.com/")
        print("    export FORMREC_KEY=your_key_here")
        print("!" * 64 + "\n")
        sys.exit(1)


# ════════════════════════════════════════════════════════════════════════════
#  ENTRY POINT
# ════════════════════════════════════════════════════════════════════════════

if __name__ == "__main__":
    if len(sys.argv) not in (2, 3):
        print(__doc__)
        print("Usage:")
        print("  python pdf_to_excel_v3.py  input.pdf")
        print("  python pdf_to_excel_v3.py  input.pdf   output.xlsx")
        print("  python pdf_to_excel_v3.py  my_folder/  output_folder/")
        sys.exit(1)

    check_keys()

    inp = sys.argv[1]
    out = sys.argv[2] if len(sys.argv) == 3 else None

    if os.path.isdir(inp):
        convert_folder(inp, out or (inp.rstrip("/\\") + "_excel_output"))
    elif os.path.isfile(inp) and inp.lower().endswith(".pdf"):
        convert_pdf(inp, out or str(Path(inp).with_suffix(".xlsx")))
    else:
        print(f"\nERROR: '{inp}' is not a valid PDF file or folder.")
        print("Make sure the path is correct and the file ends in .pdf")
        sys.exit(1)
