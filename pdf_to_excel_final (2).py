...
║  Extracts 100% of PDF: text, tables, key-values, images — exact positions   ║
╚══════════════════════════════════════════════════════════════════════════════╝
...
STEP 3 — RUN:
    python pdf_to_excel_final.py  file.pdf
    python pdf_to_excel_final.py  file.pdf   output.xlsx
    python pdf_to_excel_final.py  folder/    output_folder/
"""
...
try:
    ...
    from openpyxl.cell.cell import MergedCell          # ← needed for guard
except ImportError:
...
# "prebuilt-layout" = best model: tables + words + lines + key-value pairs
DI_MODEL = "prebuilt-layout"

# Grid: how many Excel columns/rows map across the full PDF page
# Higher = tighter position match. Lower = faster, smaller file.
GRID_COLS  = 80
GRID_ROWS  = 100
...
def is_merged(ws, row, col):
    """Return True if the cell at (row, col) is part of a merged region."""
...
def wcell(ws, row, col, val,
    ...
    """
    # Guard: never write into a merged slave cell — causes AttributeError
...
def to_col(x_in, pw):
    """x in inches → 1-based Excel column clamped to GRID_COLS."""
    return max(1, min(GRID_COLS, int(x_in / pw * GRID_COLS) + 1))


def to_row(y_in, ph):
    """y in inches → 1-based Excel row clamped to GRID_ROWS."""
...
def build_page_data(result):
    ...
    Returns dict: { page_num: {w, h, lines, tables, kvpairs, paragraphs} }
      ...
      tables, kvpairs, paragraphs — structured elements
    """
    pages = {}
    ...
    for pg in result.pages:
        ...
        if pg.lines:
            ...
            # No lines: group words by y-position into synthetic lines
            words = []
            for w in pg.words:
            ...
            # Group words that share the same approximate y
            bucket = []
            cur_y  = None
            for w in words:
    ...
    # ── Paragraphs (with role tags: title, sectionHeading, footnote …) ────────
...
def write_page(ws, pdata, page_num, fitz_doc=None, page_idx=0):
    pw = pdata["w"]
    ph = pdata["h"]
    merged_done = set()   # tracks (r1,c1,r2,c2) already merged
    ...
    for tbl in pdata["tables"]:
        ...
        if bbox:
            ...
            sc0, sr0 = 1, 1
            ec0 = min(GRID_COLS, ncols * 3)
            er0 = min(GRID_ROWS, nrows * 2)
    ...
    # ═════════════════════════════════════════════════════════════════════════
    #  B. ALL TEXT LINES  — every line placed at its exact x/y position
    # ═════════════════════════════════════════════════════════════════════════
    # Group lines by their target Excel row, then write left→right
    ...
    for line in pdata["lines"]:
        ...
        # Skip cells inside a table bounding box
        if (sr, sc) in tab_occ:
            continue
    ...
    for ex_row in sorted(row_buckets.keys()):
        ...
        for sc, ec, content, h_in in items:
            ...
            # Check for existing value (don't overwrite tables or prior text)
            if not is_merged(ws, row, sc):
                existing = ws.cell(row=row, column=sc).value
                if existing:
                    # Append with space if something already written here
            ...
            # Adjust row height to match text size
...
def convert_pdf(pdf_path, out_path):
    ...
    # Remove blank default sheet if we created named sheets
...
if __name__ == "__main__":
    if len(sys.argv) not in (2, 3):
        print(__doc__)
        print("Usage:")
        print("  python pdf_to_excel_final.py  input.pdf")
        print("  python pdf_to_excel_final.py  input.pdf   output.xlsx")
        print("  python pdf_to_excel_final.py  my_folder/  output_folder/")
        sys.exit(1)
